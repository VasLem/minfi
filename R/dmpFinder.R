...
dmpFinder.modified <- function(dat,
                               pheno,
                               cov.cols=NULL,
    ...
    pheno.array <- as.data.frame(pheno)
    suppressWarnings({
    ...
    .isMatrixBackedOrStop <- function(object, FUN) {
        if (!.isMatrixBacked(object)) {
            stop("'",
                 FUN,
                 "()' only supports matrix-backed minfi objects.",
                 call. = FALSE)
        }
    }
    type <- match.arg(type)
    ...
    pheno <- pheno.array[, 1]
    if (!is.null(cov.cols)) {
        covs <- pheno.array[, cov.cols]
        is.cat <- sapply(covs, is.factor)
        if (sum(is.cat) > 1) {
            cov.cat <- factor(apply(covs[, is.cat], 1, paste, collapse = "-"))
        } else {
            cov.cat <- factor(covs[, is.cat])
        }
        covs <- cbind(covs[!is.cat], cov.cat)
    } else {
        covs <- NULL
    }
    pheno <- factor(as.character(pheno))
    if (!is.null(covs)) {
        pheno.and.covs <- cbind(pheno, covs)
    } else {
        pheno.and.covs <- pheno
    }
    ...
    if (!is.null(covs)){
        design <- model.matrix( ~ ., data = pheno.and.covs)
    } else {
    ...
    if (type == "categorical") {
        if (shrinkVar) {
            ...
            if (is.null(covs)) {
                ...
                fit1 <- lmFit(M, model.matrix( ~ ., data = covs))
            }
            ...
            if (is.null(covs)) {
                ...
                cat.unique <- length(unique(interaction(covs[, is.cat])))
                df2 <- n - length(levels(pheno)) - cat.unique - sum(!is.cat)
            }
